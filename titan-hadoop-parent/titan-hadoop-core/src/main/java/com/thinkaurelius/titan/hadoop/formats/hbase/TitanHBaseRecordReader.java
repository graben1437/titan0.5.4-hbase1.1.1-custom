...
public class TitanHBaseRecordReader extends RecordReader<NullWritable, FaunusVertex> {
 ...
 //   public TitanHBaseRecordReader(final TitanHBaseInputFormat inputFormat, final FaunusVertexQueryFilter vertexQuery, final TableRecordReader reader, final byte[] edgestoreFamilyBytes) {
   public TitanHBaseRecordReader(final TitanHBaseInputFormat inputFormat, final FaunusVertexQueryFilter vertexQuery, final RecordReader reader, final byte[] edgestoreFamilyBytes) {
    ...
    public boolean nextKeyValue() throws IOException, InterruptedException {
        while (reader.nextKeyValue()) {
            final FaunusVertex temp = graph.readHadoopVertex(
                    configuration,
                    ((ImmutableBytesWritable)reader.getCurrentKey()).copyBytes(),
                    ((Result)reader.getCurrentValue()).getMap().get(edgestoreFamilyBytes));
            if (null != temp) {
                vertex = temp;
